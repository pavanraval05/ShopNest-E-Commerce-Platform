...
export default function Product() {
  ...
  // Filter products based on the URL parameters
  //   const filteredProducts = mens_kurta.filter((product) => {
  //     return filters.every((section) => {
  //       const searchParams = new URLSearchParams(location.search);

  //       const filterValues = searchParams.get(section.id)?.split(",") || [];
  //       return (
  //         filterValues.length === 0 || filterValues.includes(product[section.id])
  //       );
  //     });
  //   });

  //   useEffect(() => {
  //     // This useEffect will run every time the URL changes, forcing a re-render
  //   }, [location.search]);
      ...
      <div>
        ...
        >
          <Transition.Child
            ...
            leaveTo="opacity-0"
          >
            <Dialog className="fixed inset-0 bg-black bg-opacity-25" />
          </Transition.Child>
          <Transition.Child
            as="div"
